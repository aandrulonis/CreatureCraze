...
Song::Song(char* name, int name_length, vector<NoteStruct> notes, int tempo, int time_sig_num, int time_sign_denom_pow) :
    name(name), name_length(name_length), notes(notes), tempo(tempo), time_sig_num(time_sig_num), time_sig_denom_pow(time_sig_denom_pow) {}
...
Song::Song(Song &other, std::string name, const TempoStruct &tempoStruct, const TimeSignatureStruct &timeSignatureStruct,
...
bool Song::write_to_midi(const char* fname, const char* dname) {
